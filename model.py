...
class Model:
    ...
    def get_model_and_params_for_ml_problem(self):
        if self.type_ml == "r":
            ...
            param_grid_r_rf = dict([("r__n_estimators", [150, 300]), ("r__max_features", ["sqrt", "auto"]),
                                    ("r__max_depth", [3, self.n_features // 2, None]), ("r__n_jobs", [-1]),
                                    ("r__min_samples_leaf", [2, 4, 10]), ("r__verbosity ", [1])])
            ...
            param_grid_clf_log = dict([("clf__C", [0.001, 0.01, 0.1, 1, 10, 100]),
                                       ("clf___class_weight", ["balanced"]), ("clf__n_job", [-1])])
    ...
    def get_preprocessing_for_the_model_kind(self, kind):
        if kind == "linear":
            ...
            return ("ctScaler", ct), ("oneHotEncoder", CustomOneHotEncoder())
        elif kind == "tree":
    ...
    def processing(self, scoring_optimization=None, cv=3):
        config = self.config
        resultat_model = {"grid_scores": {}, "best_estim": {}, "performance_model": {}, "y_predit": {}}
        ...
        for nom_algo in config.columns.tolist():
            ...
            cv_tmp = GridSearchCV(pipe_final, param_grid=tmp_grid, scoring=scoring_optimization, cv=cv)
            cv_tmp.fit(X_train, self.y_train)
            resultat_model["grid_scores"][nom_algo] = cv_tmp.grid_scores_
            resultat_model["best_estim"][nom_algo] = cv_tmp.best_estimator_
